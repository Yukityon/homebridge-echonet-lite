...
class ELPlatform {
    ...
    return new Promise((resolve, reject) => {
      el.startDiscovery(async (err, res) => {
        ...
        for (const eoj of device.eoj) {
          ...
          // Log identification info to help user configure deviceNames
          this.log(`Discovered device: address=${address}, eoj=${JSON.stringify(eoj)}, uid=${uid}, uuid=${uuid}`)
          
          await this._addAccesory(address, eoj, uuid, uid)
        }
      })
  ...
  async _addAccesory(address, eoj, uuid, uid) {
    const registered = this.accessories.has(uuid)
    
    // Determine the name
    let name = el.getClassName(eoj[0], eoj[1])
    if (this.config.deviceNames && Array.isArray(this.config.deviceNames)) {
      // Check both UUID and UID
      const custom = this.config.deviceNames.find(d => d.id === uuid || d.id === uid)
      if (custom && custom.name) {
    ...
    // Ensure name is updated even for registered accessories if config changed
    if (registered && this.config.deviceNames && Array.isArray(this.config.deviceNames)) {
        const custom = this.config.deviceNames.find(d => d.id === uuid || d.id === uid)
        if (custom && custom.name) {
            accessory.displayName = custom.name
        }
